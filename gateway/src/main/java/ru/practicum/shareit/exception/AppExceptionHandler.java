...
public class AppExceptionHandler {
    ...
    public ResponseEntity<Response> handleValidationExceptions(MethodArgumentNotValidException ex) {
        ...
        Response response = new Response("Ошибка валидации", errors);
        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<Response> handleCustomApiException(CustomApiException ex) {
        log.warn("Выброшено исключение CustomApiException: {}", ex.getMessage());
        Response response = new Response("Ошибка HTTP-запроса", ex.getMessage());
        return ResponseEntity.status(ex.getStatus()).body(response);
    }

    @ExceptionHandler(ApiOperationException.class)
    public ResponseEntity<Response> handleApiOperationException(ApiOperationException ex) {
        log.warn("Выброшено исключение ApiOperationException: {}", ex.getMessage());
        Response response = new Response("Ошибка HTTP-запроса при создании объекта",
                ex.getMessage());
        return ResponseEntity.status(ex.getHttpStatusCode()).body(response);
    }

    @ExceptionHandler(ServerResponseException.class)
    public ResponseEntity<Response> handleServerResponseException(ServerResponseException ex) {
        log.error("Ошибка сервера: {} {}", ex.getMessage(), ex.getStatus());
        return ResponseEntity.status(ex.getStatus())
                .body(new Response("Ошибка сервера", ex.getMessage()));
    }

    @ExceptionHandler(NetworkException.class)
    public ResponseEntity<Response> handleNetworkException(NetworkException ex) {
